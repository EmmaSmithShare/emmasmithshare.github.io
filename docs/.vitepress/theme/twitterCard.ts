...
const twitterCard = (pageData: PageData, ctx: TransformPageContext) => {
    ...
    const ogImage: HeadConfig = ["meta", {property: "og:image", content: frontmatter.image || 'https://www.104070.xyz/logo/logo_light.svg'}]

    const twitterCard: HeadConfig = ["meta", {name: 'twitter:card', content: 'summary'}]
    const twitterSite: HeadConfig = ["meta", {content: '@104070Share'}]
    const twitterImage: HeadConfig = ["meta", {name: "twitter:image:src", content: frontmatter.image || 'https://www.104070.xyz/logo/logo_light.svg'}]
